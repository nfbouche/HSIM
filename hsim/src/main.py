...
def main(input_parameters):
	...
	save_fits_cube(outFile_flux_cal_noiseless, output_cube_spec_wo_back*factor_calibration[:,np.newaxis,np.newaxis]/(DIT*spaxel_area), "Flux cal Noiseless O", head)
	save_fits_cube(outFile_flux_cal_reduced, sim_reduced*factor_calibration[:,np.newaxis,np.newaxis]/(NDIT*DIT*spaxel_area), "Flux cal Reduced (O+B1+Noise1) - (B2+Noise2)", head)
